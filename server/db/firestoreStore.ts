...
export class FirestoreDataStore implements DataStore {
  ...
  private fallbackStore = new JsonDataStore();
  private useFallback = false;

  private async execWithFallback<T>(
    ...
    try {
      ...
      const isPermOrUnavail =
        err?.code === 7 ||
        err?.code === 14 ||
        err?.code === 5 ||
        /permission denied|missing or insufficient permissions|unavailable|not_found|unauthenticated/i.test(errMsg);

      if (isPermOrUnavail) {
        if (!this.useFallback) {
          console.warn(
            `[FirestoreDataStore] Firestore is unavailable or permission denied (${errMsg}). Gracefully operating via local JSON store.`
          );
  ...
  public async clearAll(orgId: string): Promise<void> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.clearAll(orgId)
    );
  }
  ...
  public async addRawRecord(record: RawSourceRecord): Promise<RawSourceRecord> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.addRawRecord(record)
    );
  }
  ...
  public async getRawRecord(id: string): Promise<RawSourceRecord | undefined> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.getRawRecord(id)
    );
  }
  ...
  public async getAllRawRecords(orgId?: string): Promise<RawSourceRecord[]> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.getAllRawRecords(orgId)
    );
  }
  ...
  public async createCanonicalEntity(
    ...
    return this.execWithFallback(
      ...
      () => this.fallbackStore.createCanonicalEntity(orgId, data)
    );
  }
  ...
  public async getCanonicalEntity(id: string): Promise<CanonicalEntity | undefined> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.getCanonicalEntity(id)
    );
  }
  ...
  public async getAllCanonicalEntities(orgId: string): Promise<CanonicalEntity[]> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.getAllCanonicalEntities(orgId)
    );
  }
  ...
  public async updateCanonicalEntity(
    ...
    return this.execWithFallback(
      ...
      () => this.fallbackStore.updateCanonicalEntity(orgId, id, payload)
    );
  }
  ...
  public async addEntityLink(link: EntityLink): Promise<EntityLink> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.addEntityLink(link)
    );
  }
  ...
  public async isPairRejected(
    ...
    return this.execWithFallback(
      ...
      () => this.fallbackStore.isPairRejected(orgId, identityKeyOrRecordId, canonicalEntityId)
    );
  }
  ...
  public async recordRejection(
    ...
    return this.execWithFallback(
      ...
      () => this.fallbackStore.recordRejection(orgId, identityKey, canonicalEntityId)
    );
  }
  ...
  public async findTopCandidatesByVector(
    ...
    return this.execWithFallback(
      ...
      () => this.fallbackStore.findTopCandidatesByVector(orgId, queryEmbedding, topN, minSimilarity, embeddingModel)
    );
  }
  ...
  public async addPendingSuggestion(suggestion: MergeSuggestion): Promise<MergeSuggestion> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.addPendingSuggestion(suggestion)
    );
  }
  ...
  public async getPendingSuggestions(orgId: string): Promise<MergeSuggestion[]> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.getPendingSuggestions(orgId)
    );
  }
  ...
  public async approveMerge(
    ...
    return this.execWithFallback(
      ...
      () => this.fallbackStore.approveMerge(orgId, suggestionId, rawRecordId, canonicalEntityId, decidedBy)
    );
  }
  ...
  public async rejectMerge(
    ...
    return this.execWithFallback(
      ...
      () => this.fallbackStore.rejectMerge(orgId, suggestionId, rawRecordId, canonicalEntityId, reason)
    );
  }
  ...
  public async getStats(orgId: string): Promise<IngestionStats> {
    return this.execWithFallback(
      ...
      () => this.fallbackStore.getStats(orgId)
    );
  }
  ...
  public async queryEntities(
    ...
    return this.execWithFallback(
      ...
      () => this.fallbackStore.queryEntities(orgId, params)
    );
  }
  ...
  public async getEntityDetails(
    ...
    return this.execWithFallback(
      ...
      () => this.fallbackStore.getEntityDetails(orgId, entityId)
    );
  }
}
