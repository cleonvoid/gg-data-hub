  ...
  public async findTopCandidatesByVector(
    ...
    try {
      if (typeof (this.canonicalEntitiesCol as any).findNearest === 'function') {
        ...
        // Over-fetch by 10x before in-memory embeddingModel filtering.
        // Once a database mixes models (e.g. legacy fallback vs gemini embeddings),
        // top vector candidates may belong to a different model; over-fetching prevents silent recall loss.
